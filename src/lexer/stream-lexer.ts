...
export class StreamLexer {
	...
	private processTextChar(c: string) {
		...
		if (this.openerState === 0) {
			if (c === "\n") {
				...
				this.openerState = 9;
				this.openerTagMatchLen = 1;
				this.openerCandidates = OPENER_TAGS;
			} else {
			...
			if (/[a-z]/i.test(c)) {
				this.openerLangWord += cLower;
				if (
					!"xml".startsWith(this.openerLangWord) &&
					!"json".startsWith(this.openerLangWord)
				) {
					failed = true;
				}
			} else if (c === " " || c === "\t") {
				if (this.openerLangWord === "xml" || this.openerLangWord === "json") {
					this.openerState = 7;
				} else {
					failed = true;
				}
			...
			this.openerCandidates = this.openerCandidates.filter(
				(t) => t[this.openerTagMatchLen] === cLower,
			);
			if (this.openerCandidates.length > 0) {
				this.openerTagMatchLen++;
				if (
					this.openerCandidates.some((t) => t.length === this.openerTagMatchLen)
				) {
					// MATCHED!
					...
					const matchWithoutNewline = leadingNewline
						? this.openerMatchedText.substring(1)
						: this.openerMatchedText;
	...
	private processToolCallChar(c: string) {
		...
		if (this.buffer.length > 100000) {
			if (this.options.onPushbackRequest) {
				this.options.onPushbackRequest(
					"Generated tool call exceeded maximum token length without closing tag. Please provide concise output.",
				);
			}
			this.buffer = "";
			this.state = "FAILED";
		...
		if (this.closeState === 0) {
			...
			this.closeCandidates = this.closeCandidates.filter(
				(t) => t[this.closeTagMatchLen] === cLower,
			);
			if (this.closeCandidates.length > 0) {
				this.closeTagMatchLen++;
				this.closeMatchLength++;
				if (
					this.closeCandidates.some((t) => t.length === this.closeTagMatchLen)
				) {
			...
			if (c === " " || c === "\t") {
				...
				this.handleToolCallClose(
					this.closeMatchStartIndex,
					this.closeMatchLength,
				);
				return;
			}
	...
	private processBufferedToolCall(rawText: string) {
		...
		if (!match) {
			if (this.options.onPushbackRequest) {
				this.options.onPushbackRequest(
					"The tool call format was invalid. Please ensure it is wrapped in <tool_call> tags.",
				);
			}
			this.state = "FAILED";
			this.buffer = "";
		...
		if (parsed && typeof parsed === "object" && parsed.name) {
			...
			if (this.options.allowedTools && this.options.allowedTools.length > 0) {
				matchedTool = this.options.allowedTools.find(
					(t) => t.function.name === parsed.name,
				);
				if (!matchedTool) {
					if (this.options.onPushbackRequest) {
						this.options.onPushbackRequest(
							`You attempted to call an unknown tool: '${parsed.name}'. Please only use tools from the provided schema.`,
						);
					}
					this.state = "FAILED";
					this.buffer = "";
				...
				if (this.options.onPushbackRequest) {
					this.options.onPushbackRequest(
						`You attempted to call a tool ('${parsed.name}'), but no tools are available. Please respond with regular text.`,
					);
				}
				this.state = "FAILED";
				this.buffer = "";
			...
			if (parsed.arguments && typeof parsed.arguments !== "object") {
				if (this.options.onPushbackRequest) {
					this.options.onPushbackRequest(
						`The arguments for tool '${parsed.name}' must be a valid JSON object.`,
					);
				}
				this.state = "FAILED";
				this.buffer = "";
			...
			if (matchedTool?.function?.parameters) {
				try {
					...
					if (!valid) {
						...
						if (this.options.onPushbackRequest) {
							this.options.onPushbackRequest(
								`Schema validation failed for tool '${parsed.name}': ${errorMsg}`,
							);
						}
						this.state = "FAILED";
						this.buffer = "";
					...
					if (this.options.onPushbackRequest) {
						this.options.onPushbackRequest(
							`Internal schema compilation failed for tool '${parsed.name}'. Check tool schema.`,
						);
					}
					this.state = "FAILED";
					this.buffer = "";
			...
			if (this.options.onPushbackRequest) {
				this.options.onPushbackRequest(
					"The JSON inside <tool_call> was malformed or missing the required 'name' property.",
				);
			}
			this.state = "FAILED";
			this.buffer = "";
