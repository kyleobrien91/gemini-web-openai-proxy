...
import type { CDPMessage, CDPTarget } from "../types/cdp.js";
...
export class CDPConnection {
	...
	async discoverTarget(): Promise<CDPTarget> {
		const response = await fetch(
			`http://${config.cdpHost}:${config.cdpPort}/json`,
		);
		if (!response.ok) {
		...
		if (!target) {
			target =
				targets.find(
					(t) => t.type === "page" && t.url.includes("gemini.google.com/app"),
				) ||
				targets.find(
			...
			if (!target) {
				const newTabRes = await fetch(
					`http://${config.cdpHost}:${config.cdpPort}/json/new?https://gemini.google.com/app`,
					{ method: "PUT" },
				);
				if (newTabRes.ok) {
		...
		if (!target) {
			throw new Error(
				"No active Gemini target found and failed to create one. Please authenticate Gemini in your browser.",
			);
		}
		...
		return new Promise((resolve, reject) => {
			...
			this.ws.on("open", async () => {
				...
				try {
					...
					reject(
						new Error(
							`Failed to enable Page domain during CDP connection: ${e}`,
						),
					);
				}
			});
