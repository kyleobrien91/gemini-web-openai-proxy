...
import type { Message, MessageContentPart } from "../types/openai.js";
...
export function parseDataUri(uri: string): { mimeType: string; bytes: Buffer } {
	...
	if (!mimeType) {
		throw new RequestExtractionError(
			`Unsupported or missing data URI MIME type: ${match[1] || "unknown"}`,
		);
	}
	...
	try {
		...
		throw new RequestExtractionError(
			`Invalid data URI payload: ${(error as Error).message}`,
		);
	}
}
...
export function validateHostnameOrIp(hostname: string): void {
	...
	if (ipFamily === 4) {
		if (isIpv4PrivateOrReserved(normalizedHostname)) {
			throw new RequestExtractionError(
				`Remote URL resolves to a private or reserved address: ${normalizedHostname}`,
			);
		}
	} else if (ipFamily === 6) {
...
export function safeLookup(
	...
	dns.lookup(hostname, lookupOpts, (err, addresses) => {
		...
		if (!addresses || addresses.length === 0) {
			return callback(
				new RequestExtractionError("Remote URL hostname could not be resolved"),
			);
		}
		...
		for (const addr of addresses) {
			...
			if (isPrivate) {
				return callback(
					new RequestExtractionError(
						`Remote URL resolves to a private or reserved address: ${addr.address}`,
					),
				);
			}
...
export async function resolveAndValidateHost(hostname: string): Promise<void> {
	...
	if (addresses.length === 0) {
		throw new RequestExtractionError(
			"Remote URL hostname could not be resolved",
		);
	}
	...
	for (const address of addresses) {
		if (
			...
			throw new RequestExtractionError(
				`Remote URL resolves to a private or reserved address: ${address.address}`,
			);
		}
	}
}
...
function safeHttpFetch(targetUrl: URL): Promise<SafeHttpResponse> {
	return new Promise((resolve, reject) => {
			...
			(res) => {
				...
				if (contentLength) {
					...
					if (
						...
						return reject(
							new RequestExtractionError(
								`Remote file exceeds maximum size of ${MAX_FILE_BYTES} bytes`,
							),
						);
					}
				}
		...
		req.setTimeout(30_000, () => {
			req.destroy(
				new RequestExtractionError(
					"Remote file fetch timed out after 30 seconds",
				),
			);
		});
...
export async function fetchRemoteFile(
	...
	for (let redirect = 0; redirect <= 5; redirect += 1) {
		if (currentUrl.protocol !== "http:" && currentUrl.protocol !== "https:") {
			throw new RequestExtractionError(
				"Only HTTP and HTTPS file URLs are supported",
			);
		}
		...
		if (res.statusCode < 200 || res.statusCode >= 300) {
			throw new RequestExtractionError(
				`Failed to fetch remote file: HTTP ${res.statusCode}`,
			);
		}
		...
		if (!mimeType) {
			throw new RequestExtractionError(
				"Unable to determine a supported MIME type for remote file",
			);
		}

		if (magicMime && headerMime && magicMime !== headerMime) {
			throw new RequestExtractionError(
				`Remote file MIME mismatch: HTTP header ${headerMime}, detected ${magicMime}`,
			);
		}
...
export function validateBytesAndMime(
	bytes: Buffer,
	...
	if (declaredMimeType) {
		if (!SUPPORTED_MIME_TYPES.has(declaredMimeType)) {
			throw new RequestExtractionError(
				`Unsupported MIME type: ${declaredMimeType}`,
			);
		}

		if (detectedMimeType && detectedMimeType !== declaredMimeType) {
			throw new RequestExtractionError(
				`File MIME mismatch: declared ${declaredMimeType}, detected ${detectedMimeType}`,
			);
		}
...
export async function resolveFilePart(
	part: Extract<MessageContentPart, { type: "file" | "image_url" }>,
): Promise<UploadableFile> {
	if (part.type === "image_url") {
		const url = part.image_url.url;
		if (url.startsWith("data:")) {
			const parsed = parseDataUri(url);
			const mimeType = validateBytesAndMime(
				parsed.bytes,
				parsed.mimeType,
				`image${extensionForMimeType(parsed.mimeType)}`,
			);
			return createExtractedFile(
				parsed.bytes,
				mimeType,
		...
		if (!remote.mimeType.startsWith("image/")) {
			throw new RequestExtractionError(
				`image_url resolved to unsupported non-image MIME type ${remote.mimeType}`,
			);
		}
		const mimeType = validateBytesAndMime(
			remote.bytes,
	...
	if (file.data) {
		...
		const filename = sanitizeFilename(
			file.name,
			`attachment${extensionForMimeType(mimeTypeCandidate || "application/pdf")}`,
		);
	...
	if (file.mime_type && normaliseMimeType(file.mime_type) !== remote.mimeType) {
		throw new RequestExtractionError(
			`Declared MIME type ${file.mime_type} does not match remote MIME type ${remote.mimeType}`,
		);
	}

	const filename = sanitizeFilename(
		file.name || remote.filename,
		remote.filename,
	);
	const mimeType = validateBytesAndMime(
		remote.bytes,
		normaliseMimeType(file.mime_type) || remote.mimeType,
		filename,
	);
	return createExtractedFile(remote.bytes, mimeType, filename, "remote-url");
}

export async function extractMultimodalMessages(
	...
	for (const message of messages) {
		...
		for (const part of message.content) {
			...
			if (fileCount > MAX_FILES_PER_REQUEST) {
				throw new RequestExtractionError(
					`A maximum of ${MAX_FILES_PER_REQUEST} files is supported per request`,
				);
			}
			...
			if (totalBytes > MAX_REQUEST_BYTES) {
				throw new RequestExtractionError(
					`Cumulative file size exceeds ${MAX_REQUEST_BYTES} bytes`,
				);
			}
