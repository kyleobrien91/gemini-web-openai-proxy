...
import {
	extractMultimodalMessages,
	RequestExtractionError,
} from "../prompt/file-extractor.js";
	...
	try {
		...
		if (
			...
			return res.status(400).json({
				error: {
					message:
						"Unsupported tool_choice value. Only 'auto' and 'none' are implicitly supported by the system prompt.",
				},
			});
		}
		...
		if (!getModel(request.model)) {
			return res
				.status(400)
				.json({ error: { message: `Unknown model: ${request.model}` } });
		}
		...
		if (request.temperature !== undefined || request.top_p !== undefined) {
			console.warn(
				"Client requested temperature or top_p, which are unsupported and ignored via Gemini Web UI proxy.",
			);
		}

		// Extract multimodal parts BEFORE locking routeMutex (SSRF, remote downloads, decoding)
		let extraction: Awaited<ReturnType<typeof extractMultimodalMessages>>;
		try {
			...
			return res
				.status(400)
				.json({ error: { message: err.message || "File extraction failed" } });
		}
		...
		try {
				...
				const lexer = new StreamLexer({
					...
					onContent: (content) => {
						if (isStream) {
							res.write(
								formatSSE(
									createContentChunk(chatId, model, content, isFirstChunk),
								),
							);
							isFirstChunk = false;
						} else {
					...
					onToolCallArg: (index, argFragment) => {
						if (isStream) {
							res.write(
								formatSSE(
									createToolArgChunk(chatId, model, index, argFragment),
								),
							);
						} else {
							if (currentToolCall)
								currentToolCall.function.arguments += argFragment;
						}
					},
					onToolCallEnd: (_index) => {
			...
			while (true) {
				...
				if (turnResult.reflectionReason && retries >= config.maxRetries) {
					throw new Error(
						`Failed to generate valid output after ${config.maxRetries} reflection attempts. Last error: ${turnResult.reflectionReason}`,
					);
				}
				...
				res.write(
					formatSSE(
						createDoneChunk(
							chatId,
							model,
							turnResult.finishReason as "stop" | "tool_calls",
						),
					),
				);
					...
					choices: [
						{
							...
							finish_reason:
								turnResult.toolCalls.length > 0 ? "tool_calls" : "stop",
						},
					],
				});
			}
			...
			if (!res.headersSent) {
				...
				if (
					...
					e.message.includes("Model switch failed")
				)
					status = 400;
