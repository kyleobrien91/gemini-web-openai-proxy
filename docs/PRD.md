...
1. **Header Delta:**
```json
data: {"id":"chatcmpl-gemini-web","object":"chat.completion.chunk","created":1740000000,"model":"gemini-3.7-flash","choices":[{"index":0,"delta":{"role":"assistant","tool_calls":[{"index":0,"id":"call_01j8abc123","type":"function","function":{"name":"execute_command","arguments":""}}]},"finish_reason":null}]}
```

2. **Argument Chunks (streamed or buffered):**
```json
data: {"id":"chatcmpl-gemini-web","object":"chat.completion.chunk","created":1740000000,"model":"gemini-3.7-flash","choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"function":{"arguments":"{\"command\": \"npm test\"}"}}]},"finish_reason":null}]}
```

3. **Finish Reason Delta:**
```json
data: {"id":"chatcmpl-gemini-web","object":"chat.completion.chunk","created":1740000000,"model":"gemini-3.7-flash","choices":[{"index":0,"delta":{},"finish_reason":"tool_calls"}]}
data: [DONE]
```

---

#### 4.3.4 OpenCode-Specific Tool Calling Nuances & Critical Edge Cases

OpenCode (and modern agentic harnesses like Cline / Roo Code) enforce strict validation on tool calling streams. The proxy implements the following explicit mechanisms to ensure 100% compatibility:

1. **Parallel / Multi-Tool Call Handling:**
   - OpenCode frequently requests reading or searching multiple files in parallel. When Gemini emits multiple consecutive `<tool_call>` blocks (e.g. `<tool_call>...</tool_call><tool_call>...</tool_call>`), the proxy parses each block into an indexed entry (`index: 0`, `index: 1`, `index: 2`, etc.) in the `tool_calls` delta array.
   - All tool calls in the turn must be closed before emitting `finish_reason: "tool_calls"`.

2. **Deterministic `tool_call_id` Lifecycle:**
   - Every tool call generated by the proxy is assigned a compliant unique ID: `call_<16-char-alphanumeric>` (e.g. `call_a1b2c3d4e5f6g7h8`).
   - When OpenCode sends subsequent turns with `role: "tool"` and `tool_call_id: "call_..."`, the proxy tracks and correlates these IDs back to the flattened context so the model understands exactly which tool returned which output.

3. **Initial Argument Initialization (`arguments: ""`):**
   - OpenAI SDKs and OpenCode's Zod/JSON parsers crash if the first `tool_calls` delta omits `function.arguments` or sets it to `null`.
   - The first chunk that introduces a tool MUST declare `function: { name: "...", arguments: "" }`.

4. **CoT / Explanatory Text Before Tool Call:**
   - OpenCode supports models explaining their plan before calling a tool.
   - The proxy streams all text tokens as standard `delta.content` chunks up until `<tool_call>` is encountered.
   - Once `<tool_call>` begins, `delta.content` emission ceases, and output transitions exclusively to `delta.tool_calls`.

5. **Strict `finish_reason` Rules:**
   - If at least one tool call was emitted in the turn, `finish_reason` MUST be `"tool_calls"`.
   - If no tool call was emitted, `finish_reason` MUST be `"stop"`.
   - Never emit `"length"` unless the browser context hard-truncated.

6. **Argument Escaping & Code Block Normalization:**
   - File edit tools (`write_to_file`, `replace_file_content`) contain raw code with unescaped quotes, markdown, and newlines.
   - The XML parser strips any accidental markdown wrapper tags inside `<tool_call>` (e.g. ````json ... ````) and normalizes escaped strings into a valid JSON argument object before emitting.

7. **Empty/No-Op Response Fallback:**
   - If Gemini produces an empty response or only whitespace without calling a tool, the proxy emits a minimal fallback message (`"I have completed the task."`) with `finish_reason: "stop"` to prevent OpenCode hanging in an infinite polling loop.
